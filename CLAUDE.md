...
- Error handling: `components/ErrorBoundary.tsx` — class-based React error boundary. Used without props (full-page fallback) around `<Outlet />` in `App.tsx`, and with `compact` prop (inline fallback) around individual chart components in `Home.tsx` and `Report.tsx`
- 404 handling: `pages/NotFound.tsx` — catch-all `path: '*'` route inside the `App` layout so the NavBar stays visible
