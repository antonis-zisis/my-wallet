...
- **Writing a comment → don't, unless it is absolutely necessary.** Default to no comment. Code should explain itself through clear names and small functions; a comment that restates _what_ the code does is noise and must be deleted. The only comments worth keeping explain a _why_ the code cannot express on its own — a non-obvious constraint, a workaround for an external bug, or a deliberate-looking-wrong choice — and even then, prefer encoding the intent in a well-named function or variable first. Never add JSDoc/function-doc blocks that narrate behaviour, and never add comments to tests describing their setup or expectations; the test name and assertions are the documentation. If you think a comment is needed, the bar is: a competent reader would get it _wrong_ without it.
