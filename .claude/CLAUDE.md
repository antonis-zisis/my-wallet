...
- Types: `types/` — shared TypeScript types per domain (`report.ts`, `subscription.ts`, `contract.ts`, `netWorth.ts`, `transaction.ts`) plus `sort.ts` (shared sort-direction type), `currency.ts` (`CURRENCIES`, `CURRENCY_CONFIG`, `CURRENCY_OPTIONS`) `plan.ts` (`PLANS`, `PlanEntitlements`, `PLAN_LIMIT_LABELS`) and `billing.ts` (`BILLING_INTERVALS`, `PlanPrice`, `PlanStatus`)
- Utils: `utils/` — pure helpers: `formatMoney`, `formatAmount`, `formatDate`, `formatDateForInput`, `formatMonth`, `formatRelativeTime`, `abbreviateReportTitle`, `getInitials`, `getNextRenewalDate`, `getDaysUntil`, `getSubscriptionLogoUrl`, `isSafeUrl`, `exportReportToCsv`, `groupEntriesByCategory`, `isActiveTrial`, `renewalDisplay`, `formatSubscriptionCountdown`, `getPlanLimitMessage`, `formatPlanPrice`, `redirectTo` (domain-specific draft/input builders like `buildContractInput` live under `hooks/<domain>/selectors/`)
...
- Billing: `lib/stripe.ts` exports the Stripe client (null when unconfigured) and `requireStripe()`; `lib/billing/` owns webhook verification, idempotency and the subscription→user mapping, so `routes/` never reaches into `graphql/`
...
Plans live in the **user** domain: `Query.plans` / `Query.planUsage` / `Mutation.selectPlan` on the server, `hooks/plan/` (`usePlan`, `usePlanData`, `usePlanGate`, `usePlanUsage`) and the `SelectPlan` page on the web — see **Plan model** below. Payment lives in its own **billing** domain (`apps/server/src/graphql/billing/`, `graphql/billing.ts` and `hooks/billing/` on the web) — see **Billing model**.
...
**Billing model:** Stripe Checkout for buying Pro, the Stripe billing portal for everything after. Pro is sold monthly or yearly; `Query.planPrices` reads both prices from Stripe at request time, so no amount is ever hardcoded in the app and changing a price in the Stripe dashboard is the whole deploy. Every billing env var is optional (`lib/env.ts`): without them the server boots, `planPrices` returns `[]`, and the Pro call to action reads "Not available yet" instead of erroring.

**The webhook is the only writer of `plan`.** `POST /webhooks/stripe` is mounted in `index.ts` before `express.json()` (Stripe signs the raw body) and outside `authMiddleware` — it is an intentionally unauthenticated route, authenticated by its signature instead of a JWT. `lib/billing/handleStripeWebhook.ts` verifies with `stripe.webhooks.constructEvent`, records the event id in `stripe_events` for idempotency (a redelivery is a no-op, because Stripe retries), then dispatches. `applyStripeSubscription` maps one Stripe subscription onto the user row and is the single place plan state is decided; `checkout.session.completed` retrieves the subscription and goes through the same path rather than duplicating the mapping. Note `current_period_end` lives on the subscription's _items_ in this SDK version, not on the subscription.

Status mapping, and what each status grants:

| Stripe status                                          | `planStatus` | `plan` |
| ------------------------------------------------------ | ------------ | ------ |
| `active`                                               | `ACTIVE`     | `PRO`  |
| `trialing`                                             | `TRIALING`   | `PRO`  |
| `past_due`                                             | `PAST_DUE`   | `PRO`  |
| `canceled`, `unpaid`, `incomplete*`, `paused`, unknown | `CANCELED`   | `FREE` |

A failed payment keeps Pro on: Stripe retries for about two weeks, and a card that merely expired should not suddenly cap someone's data entry. `PastDueBanner` (rendered in `App.tsx`) says so in as many words and points at the portal. `incomplete` maps to `CANCELED` deliberately — that subscription never had a successful payment, so it must not grant Pro.

`selectPlan` is now the downgrade path only: it refuses `PRO` (checkout owns that) and refuses `FREE` while `stripeSubscriptionId` is set, because stripping the plan locally while Stripe keeps charging is the worst possible outcome. Cancelling goes through the portal, Stripe sends `customer.subscription.updated` with `cancel_at_period_end`, and access ends when the period does.

Web side: `hooks/billing/` holds `useBillingData` (prices + checkout redirect), `useBillingPortal` (portal redirect), `useBillingStatus` (read-only status off `useUser`) and `useCheckoutCompletion`. That last one earns its place: Stripe returns the customer to `/select-plan?checkout=success` before the webhook has necessarily landed, so the page shows "Finishing up your upgrade…" and refetches `me` on a bounded backoff (1s, 1.5s, 2.5s, 4s, 6s, then a "check again" panel) rather than dropping them on a Free dashboard. Redirects go through `utils/redirectTo.ts` so they can be mocked. Plan prices are formatted by `utils/formatPlanPrice.ts` and deliberately **not** rendered through `<MoneyAmount />` — a price list is not the user's money and must stay visible in privacy mode.

Local development: `stripe listen --forward-to localhost:4000/webhooks/stripe` prints the `STRIPE_WEBHOOK_SECRET` to put in `apps/server/.env`, and test card `4242 4242 4242 4242` completes checkout.
